...
OPTIONS:
    ...
    -o OUT_PREFIX   Output prefix. output files will be:
                        "%s_%s_R%d.fq" % (OUT_PREFIX, bcd header, read num)
...
def make_bcd_tries(sf, K=1):
    ...
    for k in range(K+1):
        ...
        for hdr, seq in sf:
            files[seq] = (open("out_{}_R1.fq".format(seq), "w"),
                    open("out_{}_R2.fq".format(seq), "w"))
        ...
        tries.append(trie)
    files[None] = (open("out_NOBCD_R1.fq", "w"), open("out_NOBCD_R2.fq", "w"))
    return (tries, files)
